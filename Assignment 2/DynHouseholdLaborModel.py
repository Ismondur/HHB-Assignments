...
class DynHouseholdLaborModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        # taxes
        par.tax_scale = 2.28 # from Borella et al. (2023), singles: 1.765038
        par.tax_pow = 0.0861765 # from Borella et al. (2023), singles: 0.0646416

        par.tax_scale_indiv = 1.75
        par.tax_pow_indiv = 0.0646416
    ...
    def consumption(self,hours1,hours2,capital1,capital2):
        ...
        if par.joint_tax:
            ...
            tax_hh = self.tax_func_indiv(income1) + self.tax_func_indiv(income2) 
    ...
    def tax_func_indiv(self,income):
        par = self.par
        rate = 1-par.tax_scale_indiv*(income+0.5)**(-par.tax_pow_indiv)
        return rate*(income+0.5)
    ...
    def util(self,hours1,hours2,capital1,capital2):
        ...
        if par.mu == False:
            ...
            return 0.5*util_cons - par.mu*(util_hours1) - (1-par.mu)*(util_hours2)
    ...
    def simulate(self):
        ...
        for i in range(par.simN):
            ...
            for t in range(par.simT):
                ...
                hh_income = sim.income1[i,t]+sim.income2[i,t]
                if par.joint_tax:
                    sim.tax_revenue[i,t] = self.tax_func(hh_income)
                else:
                    sim.tax_revenue[i,t] = self.tax_func_indiv(sim.income1[i,t])+ self.tax_func_indiv(sim.income2[i,t])
